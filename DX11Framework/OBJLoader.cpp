							  ...
							  std::vector<XMFLOAT3>& outNormals)
{
	...
	for(int i = 0; i < numVertices; ++i) //For each vertex
	{
		...
		{
			outVertices.push_back(vertex.m_pos);
			outTexCoords.push_back(vertex.m_texCoords);
...
MeshData OBJLoader::Load(char const * filename, ID3D11Device* _pd3dDevice, bool invertTexCoords)
{
	...
	if(!binaryInFile.good())
	{
		...
		else
		{
			...
			{
				finalVerts[i].m_pos = meshVertices[i];
				finalVerts[i].m_normal = meshNormals[i];
				finalVerts[i].m_texCoords = meshTexCoords[i];
			}
